...
public abstract class Page implements HttpHandler {
	...
	private void workIt(HttpExchange t) {
		...
		String redirectLocation = values.get("Location");
		...
		case "GET":
			try {
				byte[] html = getBody(url, values, t);
				if (html != null)
					response.write( html );
				else{
	...
	private void getValues(HttpExchange t, Map<String, String> values) {
		...
		while (in > 0) {
			...
			String parm = body.substring(0, in), value;
	...
	private void postValues(HttpExchange t, Map<String, String> values) throws PageException {
		...
		try (Scanner s = new Scanner(t.getRequestBody())) {
			String body = URLDecoder.decode( s.useDelimiter("\\A").next(), "utf-8" );
	...
	/**
	 * execute post is called before getBody() in the case of user post.
	 * If successfully, client will get a 302 and be redirect to same page (or "redirect" field in get or post)
	 * If not, getBody() is called and it should show again the form so the user will insert it again 
	 ...
	 */
	public abstract void executePost(String url[], Map<String, String> valori, HttpExchange t) throws PageException;
	 ...
	 */
	public abstract byte[] getBody(String url[], Map<String, String> valori, HttpExchange t) throws PageException;
}
